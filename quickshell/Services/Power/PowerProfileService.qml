...
Singleton {
  ...
  property bool mistletoePerformanceMode: false
  ...
  // Mistletoe Performance Mode
  // - Turning shadow off
  // - Turning animation off
  // - Do Not Disturb
  function toggleMistletoePerformance() {
    mistletoePerformanceMode = !mistletoePerformanceMode;
  }

  function setMistletoePerformance(value) {
    mistletoePerformanceMode = value;
  }

  onMistletoePerformanceModeChanged: {
    if (mistletoePerformanceMode) {
      ToastService.showNotice("Mistletoe Performance", "Performance mode enabled", "rocket");
    } else {
      ToastService.showNotice("Mistletoe Performance", "Performance mode disabled", "rocket-off");
    }
  }
}

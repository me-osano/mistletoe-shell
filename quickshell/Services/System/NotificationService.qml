...
Singleton {
  ...
  property string historyFile: Quickshell.env("MISTLETOE_NOTIF_HISTORY_FILE") || (Settings.cacheDir + "notifications.json")

  // State
  property real lastSeenTs: 0
  // Volatile property that doesn't persist to settings (similar to mistletoePerformanceMode)
  ...
  function handleNotification(notification) {
    ...
    if (root.doNotDisturb || PowerProfileService.mistletoePerformanceMode)
      return;
  ...
  function checkMediaToast() {
    ...
    if (doNotDisturb || PowerProfileService.mistletoePerformanceMode)
      return;

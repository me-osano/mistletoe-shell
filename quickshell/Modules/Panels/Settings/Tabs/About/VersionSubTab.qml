...
ColumnLayout {
  ...
  function normalizeVersion(versionString) {
    if (!versionString)
      return "";
    return versionString.trim().replace(/^v/i, "").replace(/-git$/i, "");
  }

  // Update status: compare normalized versions
  readonly property string installedBase: root.normalizeVersion(root.currentVersion)
  readonly property string latestBase: root.normalizeVersion(root.latestVersion)
  readonly property bool updateAvailable: {
    if (!root.latestBase || !root.installedBase)
      return false;
    return root.latestBase !== root.installedBase && !root.isGitVersion;
  }
  readonly property bool isUpToDate: {
    if (!root.latestBase || !root.installedBase)
      return false;
    return root.latestBase === root.installedBase;
  }
  ...
  Component.onCompleted: {
    versionFileProcess.running = true;
    ...
    if (root.isGitVersion) {
      ...
      if (HostService.isNixOS) {
        ...
        if (shellDir) {
          // Extract commit hash from path like: /nix/store/...-mistletoe-shell-2025-11-30_225e6d3/share/mistletoe-shell
          // Pattern matches: mistletoe-shell-YYYY-MM-DD_<commit_hash>
          var match = shellDir.match(/mistletoe-shell-\d{4}-\d{2}-\d{2}_([0-9a-f]{7,})/i);
          if (match && match[1]) {
  ...
  Process {
    id: versionFileProcess
    command: ["cat", Quickshell.shellDir + "/VERSION"]
    running: false

    onExited: function (exitCode) {
      if (exitCode === 0) {
        const versionText = stdout.text.trim();
        if (versionText) {
          root.currentVersion = versionText;
          Logger.d("VersionSubTab", "Loaded version from VERSION file:", root.currentVersion);
        }
      } else {
        Logger.d("VersionSubTab", "VERSION file not found/readable at:", Quickshell.shellDir + "/VERSION");
      }
    }
  ...
  Process {
    id: pacmanProcess
    command: ["pacman", "-Q", "mistletoe-shell-git"]
    running: false
    ...
    onExited: function (exitCode) {
      ...
      if (exitCode === 0) {
        ...
        var match = output.match(/mistletoe-shell-git\s+(.+)/);
        if (match && match[1]) {
      ...
      GridLayout {
        ...
        NText {
          ...
          MouseArea {
            ...
            onClicked: {
              if (root.commitInfo) {
                Quickshell.execDetached(["xdg-open", "https://github.com/me-osano/mistletoe-shell/commit/" + root.commitInfo]);
              }
            }
          }
        }
  ...
  GridLayout {
    ...
    NButton {
      ...
      Layout.alignment: Qt.AlignHCenter
      onClicked: {
        Quickshell.execDetached(["xdg-open", "https://buymeacoffee.com/mistletoe"]);
